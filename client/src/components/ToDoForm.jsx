...
function ToDoForm() {
    ...
    // Effect hook to set form data when a toDo is being edited
    useEffect(() => {
        if (toDo) {
            setToDoData(prev => {
                return { ...prev, task: toDo.task, category: toDo.category };
            });
            taskInput.current.focus();  // Focus the task input field
        }
    }, [toDo]);
    ...
    // Function to handle the editing of an existing task
    const onEdit = (e) => {
        e.preventDefault();
    ...
    // Function to handle the form submission (prevents default behavior)
