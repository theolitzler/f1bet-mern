    ...
    for (const [betId, bet] of Object.entries(betsByUser)) {
      ...
      for (const prediction of bet.predictions) {
        ...
        if (actualPosition === prediction.predictedPosition) {
          ...
          Math.abs(actualPosition - prediction.predictedPosition) === 1
        ) {
...
const updateBetScore = (betId, score) => {
  return new Promise((resolve, reject) => {
    const query = `UPDATE bets SET score = ?, is_scored = 1 WHERE id = ?`;
...
const verifyHasBetForRace = (userId, raceId) => {
  return new Promise((resolve, reject) => {
    const query = `SELECT id FROM bets WHERE user_id = ? AND race_id = ?`;
    db.get(query, [userId, raceId], (err, row) => {
      if (err) reject(err);
      resolve(!!row); // Returns true if a bet exists, false otherwise
    });
  });
};

module.exports = { addBet, getBetsByUser, getBetById, calculateBetScores, verifyHasBetForRace };
